...
- メール/パスワード・Google・Apple の3方式でログイン可能
...
複雑な計算ロジックが少ないため、Jestによるユニットテストは導入していません。代わりに、日付の境界が絡む処理（AI分析の週次実行制限、ヒストリーカレンダーの月切り替えなど）は、実機の日付を変更してSupabase上のデータと合わせて動作確認を行っています。
